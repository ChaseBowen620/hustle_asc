...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        students = list(
            Student.objects.all().order_by('last_name', 'first_name')
        )
        if show_attendance:
            for s in students:
        ...
        for s in students:
            a = (getattr(s, 'a_number', '') or '').strip().lower()
            if a:
        ...
        self.stdout.write(self.style.WARNING('1. Same A-number — multiple accounts (definite duplicate):'))
        ...
        if not a_dup:
            ...
            for a_num, grp in sorted(a_dup.items()):
                ...
                for s in grp:
                    line = f'      id={s.id}  {s.first_name} {s.last_name}'
                    if show_attendance:
                        line += f'  attendances={getattr(s, "_att_count", "?")}'
                    self.stdout.write(line)
                self.stdout.write('')

        # 2. Same first+last name, different accounts (likely same person, two signups)
        self.stdout.write(self.style.WARNING('2. Same first + last name — different accounts (possible duplicate):'))
        name_dup = {k: v for k, v in by_name.items() if len(v) > 1}
        if not name_dup:
            self.stdout.write('   None found.\n')
        else:
            for (fn, ln), grp in sorted(name_dup.items(), key=lambda x: (x[0][1], x[0][0])):
                a_nums = [(getattr(s, 'a_number', '') or '').strip().lower() for s in grp]
                ...
                for s in grp:
                    line = f'      id={s.id}  A-number={getattr(s, "a_number", "") or "(none)"}'
                    if show_attendance:
                        line += f'  attendances={getattr(s, "_att_count", "?")}'
                    self.stdout.write(line)
                self.stdout.write('')
        ...
        # Summary
        total_dup_groups = len(a_dup) + len([k for k, v in name_dup.items()
            if not (len(set((getattr(s, 'a_number', '') or '').strip().lower() for s in v)) == 1 and (getattr(v[0], 'a_number', '') or '').strip() and len(v) > 1)])
